import { MediaQueries, Breakpoints, Spacing } from './types'

export const breakpointMap: { [key: string]: number } = {
  xs: 370,
  sm: 576,
  md: 730,
  lg: 968,
  xl: 1080,
  xxl: 1200,
}

const breakpoints: Breakpoints = Object.values(breakpointMap).map((breakpoint) => `${breakpoint}px`)

const mediaQueries: MediaQueries = {
  xs: `@media screen and (min-width: ${breakpointMap.xs}px)`,
  sm: `@media screen and (min-width: ${breakpointMap.sm}px)`,
  md: `@media screen and (min-width: ${breakpointMap.md}px)`,
  lg: `@media screen and (min-width: ${breakpointMap.lg}px)`,
  xl: `@media screen and (min-width: ${breakpointMap.xl}px)`,
  xxl: `@media screen and (min-width: ${breakpointMap.xxl}px)`,
  nav: `@media screen and (min-width: ${breakpointMap.lg}px)`,
}

export const shadows = {
  level1: '0px 2px 12px -8px rgba(25, 19, 38, 0.1), 0px 1px 1px rgba(25, 19, 38, 0.05)',
  active: '0px 0px 0px 1px #0098A1, 0px 0px 4px 8px rgba(31, 199, 212, 0.4)',
  success: '0px 0px 0px 1px #19E285, 0px 0px 0px 4px rgba(49, 208, 170, 0.2)',
  warning: '0px 0px 0px 1px #FE3B33, 0px 0px 0px 4px rgba(237, 75, 158, 0.2)',
  focus: '0px 0px 0px 1px #8A6FFE, 0px 0px 0px 4px rgba(138, 111, 254,  0.6)',
  inset: 'inset 0px 2px 2px -1px rgba(74, 74, 104, 0.1)',
  tip: 'inset 0 0 2px 1px rgba(74, 74, 104, 0.5)',
}

export const buttonShadows = (color: string, size: string): string => `
1px 0px ${size} ${color},
-1px 0px ${size} ${color},
0px 1px ${size} ${color},
0px -1px ${size} ${color};`

export const textShadows = {
  level1: '0px 2px 12px rgba(25, 19, 38, 0.1), 0px 1px rgba(25, 19, 38, 0.05)',
  active: '0px 0px 0px #0098A1, 0px 0px 4px rgba(31, 199, 212, 0.4)',
  success: '0px 0px 0px #19E285, 0px 0px 0px rgba(49, 208, 170, 0.2)',
  warning: '0px 0px 0px #FE3B33, 0px 0px 0px rgba(237, 75, 158, 0.2)',
  focus: '0px 0px 0px #00B356, 0px 0px 0px rgba(22, 141, 94, 0.6)',
  inset: '0px 2px 2px rgba(74, 74, 104, 0.2)',
}

const spacing: Spacing = [0, 4, 8, 16, 24, 32, 48, 64]

const radii = {
  small: '2px',
  default: '2px',
  card: '2px',
  circle: '50%',
}

const zIndices = {
  dropdown: 10,
  modal: 100,
}

export default {
  siteWidth: 1200,
  breakpoints,
  mediaQueries,
  textShadows,
  spacing,
  shadows,
  radii,
  zIndices,
}
